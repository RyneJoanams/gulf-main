...
exports.createPayment = async (req, res) => {
    const { patientName, modeOfPayment, accountNumber, amountPaid, commission, xrayPayment, amountDue, paymentStatus } = req.body;

    // Validate required fields
    if (!patientName || !modeOfPayment || !accountNumber || amountPaid == null || !commission || !xrayPayment || amountDue == null || !paymentStatus) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    const newPayment = new accounts({
        patientName,
        modeOfPayment, // Moved field
        accountNumber,
        amountPaid, // Moved field
        commission, // Moved field
        xrayPayment, // Moved field
        amountDue, // Moved field
        paymentStatus,
    });
...
exports.updatePayment = async (req, res) => {
    ...
    try {
        ...
        const updatedPayment = await accounts.findByIdAndUpdate(
            req.params.id,
            { patientName, modeOfPayment, accountNumber, amountPaid, commission, xrayPayment, amountDue, paymentStatus }, // Moved fields
